...
export interface ParsedInvite {
  ...
  salt?: string; // list salt (list invites only)
}

/** Parse a pasted link OR a bare hash (#/f/…, #/l/…) into an invite. */
export function parseInvite(text: string): ParsedInvite | null {
  ...
  try {
    ...
    const salt = m[4] ? decodeURIComponent(m[4]).trim() : "";
    return { kind: "list", username: b, name: a, salt: salt || undefined };
  } catch {
    return null;
  }
}
